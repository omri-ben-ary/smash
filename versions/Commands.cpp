...
namespace SyscallErrorMessages
{
    ...
    static const char *WRITE_FAILED = "smash error: write failed";
    static const char *READ_FAILED = "smash error: read failed";
...
void PipeCommand::execute() {
    ...
    char command_done = '1';
    if(write(fd_array[1], &command_done, 1) != 1)
    {
        dup2(std_fd_array[0],fd_array[0]);
        dup2(std_fd_array[1],fd_array[1]);
        close(std_fd_array[0]);
        close(std_fd_array[1]);
        perror(SyscallErrorMessages::WRITE_FAILED);
        return;
    }
    char read_buffer[COMMAND_ARGS_MAX_LENGTH + 1];
    std::string command2_line = command2;
    int read_count = read(fd_array[0], read_buffer, COMMAND_ARGS_MAX_LENGTH + 1);
    if(read_count < 1)
    {
        dup2(std_fd_array[0],fd_array[0]);
        dup2(std_fd_array[1],fd_array[1]);
        close(std_fd_array[0]);
        close(std_fd_array[1]);
        perror(SyscallErrorMessages::READ_FAILED);
        return;
    }
    else if(read_count > 1)
    {
        command2_line += " " + std::string(read_buffer, read_count - 1);
    }
    ...
    Command *cmd2 = smash.CreateCommand(command2_line.c_str());
    smash.updateJobList();
    if(cmd2 != nullptr)
    {
        cmd2->execute();

...
void KillCommand::execute()
{
    ...
    else
    {
        ...
        if ((sig_num == SIGSTOP) || (sig_num == SIGTSTP)) //added
        {
            job->stopJob();
        }
        ...
        {
            job_pid = job->getJobPID(); //ADDED
        }

        jobs->sendSignalToJob(job_id, sig_num, true);
        // ADDED
        if ((sig_num == SIGKILL) || (sig_num == SIGTERM) || (sig_num == SIGINT) || (sig_num == SIGQUIT) || (sig_num == SIGABRT))
        {
            jobs->removeFromListJobByPid(job_pid);
        }
        SmallShell& smash  = SmallShell::getInstance();
        if (this->isTimeout())
        {
